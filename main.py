...
class Route:
    def __init__(self, city_indices: List[int], distance_matrix: DistanceMatrix) -> None:
    ...
    def length(self) -> float:
        ...
        for i in range(len(self.city_indices)):
            total_distance += self.distance_matrix.distance(self.city_indices[i - 1], self.city_indices[i])
        return total_distance

    def mutate(self, mutation_rate: float) -> None:
        if random.random() < mutation_rate:
            idx1, idx2 = random.sample(range(len(self.city_indices)), 2)
            self.city_indices[idx1], self.city_indices[idx2] = self.city_indices[idx2], self.city_indices[idx1]

    @staticmethod
    def crossover(parent1: 'Route', parent2: 'Route') -> Tuple['Route', 'Route']:
        child1, child2 = Route.crossover_helper(parent1, parent2), Route.crossover_helper(parent2, parent1)
...
class Population:
    def __init__(self, size: int, distance_matrix: DistanceMatrix) -> None:
        self.size = size
        self.routes = [Route(self.random_route(len(distance_matrix.matrix)), distance_matrix) for _ in range(size)]
...
class GeneticAlgorithm:
    def __init__(self, distance_matrix: DistanceMatrix, population_size: int, generations: int, mutation_rate: float, tournament_size: int) -> None:
        self.distance_matrix = distance_matrix
    ...
    def run(self) -> Route:
        ...
        print("Best initial route:", initial_best.city_indices)
...
def main():
    ...
    ga = GeneticAlgorithm(distance_matrix, population_size, generations, mutation_rate, tournament_size)
    best_route = ga.run()
    end = time.perf_counter()

    print("Best route found:", best_route.city_indices)
